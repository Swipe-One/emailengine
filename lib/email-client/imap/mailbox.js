...
class Mailbox {
    ...
    async onExists(event) {
        // Use global Sentry instance from the IMAP worker thread
        const Sentry = global.Sentry || null;
        
        if (!Sentry) {
            return this._onExistsInternal(event);
        }

        return await Sentry.startSpan(
            {
                name: 'ImapMailbox.onExists',
                attributes: {
                    // Event Context
                    trigger: 'new_message_detection',
                    eventType: 'IMAP_EXISTS',
                    
                    // Mailbox Context
                    account: this.connection?.account,
                    path: this.path,
                    specialUse: this.listingEntry?.specialUse,
                    
                    // Message Detection
                    previousMessages: event?.previousCount,
                    currentMessages: event?.count,
                    newMessageCount: (event?.count || 0) - (event?.previousCount || 0),
    ...
    /**
     * Internal implementation of onExists (extracted for Sentry tracking)
     * @private
     */
    async _onExistsInternal(event) {
    ...
    async processNew(messageData, options, canSync, storedStatus) {
        ...
        if (!Sentry) {
            // If Sentry is not initialized, proceed without tracking
            return this._processNewInternal(messageData, options, canSync, storedStatus);
        }

        return await Sentry.startSpan(
            {
                name: 'ImapMailbox.processNew',
                attributes: {
                    // Message Context
                    account: this.connection?.account,
                    messageUid: messageData?.uid,
                    path: this.path,
                    flags: messageData?.flags ? Array.from(messageData.flags).join(',') : undefined,
                    canSync: canSync,
                    
                    // Processing Context
                    clientType: 'IMAP',
                    messageProcessingReason: 'new_message_notification',
                    hasOptions: !!options,
                    hasStoredStatus: !!storedStatus,
                ...
                // ✅ Removed forceTransaction: true to make this a child span
            },
            async () => {
                return this._processNewInternal(messageData, options, canSync, storedStatus);
            }
    ...
    /**
     * Internal implementation of processNew (extracted for Sentry tracking)
     * @private
     */
    async _processNewInternal(messageData, options, canSync, storedStatus) {
    ...
    async fullSync() {
        ...
        if (!Sentry) {
            return this._fullSyncInternal();
        }

        const imapIndexer = this.imapIndexer;
        const mailboxStatus = this.getMailboxStatus();
        
        return await Sentry.startSpan(
            {
                name: 'ImapMailbox.fullSync',
                attributes: {
                    // Sync Context
                    syncType: 'full',
                    indexerType: imapIndexer,
                    
                    // Mailbox Context
                    path: this.path,
                    messageCount: mailboxStatus?.messages,
    ...
    /**
     * Internal implementation of fullSync
     * @private
     */
    async _fullSyncInternal() {
        const imapIndexer = this.imapIndexer;
    ...
    async partialSync(storedStatus) {
        ...
        if (!Sentry) {
            return this._partialSyncInternal(storedStatus);
        }
        ...
        return await Sentry.startSpan(
            {
                ...
                attributes: {
                    ...
                    // Mailbox Context  
                    path: this.path,
                    currentMessages: mailboxStatus?.messages,
                    storedMessages: storedStatus?.messages,
                    newMessageCount: mailboxStatus?.messages - (storedStatus?.messages || 0),
                    uidNext: mailboxStatus?.uidNext,
    ...
    /**
     * Internal implementation of partialSync
     * @private
     */
    async _partialSyncInternal(storedStatus) {
        const imapIndexer = this.imapIndexer;
    ...
    async runFastSync(storedStatus) {
        ...
        if (!Sentry) {
            return this._runFastSyncInternal(storedStatus);
        }
        ...
        return await Sentry.startSpan(
            {
                ...
                attributes: {
                    ...
                    uidNext: mailboxStatus?.uidNext,
                    storedUidNext: storedStatus?.uidNext,
                    modseq: mailboxStatus?.highestModseq,
                    storedModseq: storedStatus?.highestModseq,
                    
                    // Expected Processing
                    expectedNewMessages: Math.max(0, (mailboxStatus?.messages || 0) - (storedStatus?.messages || 0)),
    ...
    /**
     * Internal implementation of runFastSync
     * @private
     */
    async _runFastSyncInternal(storedStatus) {
        storedStatus = storedStatus || (await this.getStoredStatus());
        let mailboxStatus = this.getMailboxStatus();
    ...
    async runPartialSync(storedStatus) {
        ...
        if (!Sentry) {
            return this._runPartialSyncInternal(storedStatus);
        }
        ...
        return await Sentry.startSpan(
            {
                name: 'ImapMailbox.runPartialSync',
                attributes: {
                    // Sync Context
                    syncMethod: 'partial',
                    syncType: 'incremental',
                    supportsCondstore: this.connection?.imapClient?.enabled?.has('CONDSTORE') || false,
                    ...
                    // Change Detection
                    messageCountChange: (mailboxStatus?.messages || 0) - (storedStatus?.messages || 0),
                    modseqChanged: storedStatus?.highestModseq && mailboxStatus?.highestModseq > storedStatus.highestModseq,
    ...
    /**
     * Internal implementation of runPartialSync
     * @private
     */
    async _runPartialSyncInternal(storedStatus) {
        storedStatus = storedStatus || (await this.getStoredStatus());
        let mailboxStatus = this.getMailboxStatus();
    ...
    async runFullSync() {
        ...
        if (!Sentry) {
            return this._runFullSyncInternal();
        }
        ...
        return await Sentry.startSpan(
            {
                name: 'ImapMailbox.runFullSync',
                attributes: {
                    // Sync Context
                    syncMethod: 'full',
                    syncType: 'complete',
                    syncReason: 'comprehensive_scan',
                    ...
                    // Performance Context
                    isLargeMailbox: (mailboxStatus?.messages || 0) > 1000,
                    expectedProcessing: 'all_messages',
    ...
    /**
     * Internal implementation of runFullSync
     * @private
     */
    async _runFullSyncInternal() {
        let fields = { uid: true, flags: true, modseq: true, emailId: true, labels: true, internalDate: true };
        let opts = {};
    ...
    async publishSyncedEvents(storedStatus) {
        ...
        if (!Sentry) {
            return this._publishSyncedEventsInternal(storedStatus);
        }
        
        return await Sentry.startSpan(
            {
                name: 'ImapMailbox.publishSyncedEvents',
                attributes: {
                    // Event Context
                    ...
                    // Processing Context
                    hasStoredStatus: !!storedStatus,
                    messageCount: storedStatus?.messages,
    ...
    /**
     * Internal implementation of publishSyncedEvents
     * @private
     */
    async _publishSyncedEventsInternal(storedStatus) {
    ...
    async onOpen() {
        ...
        if (!Sentry) {
            // If Sentry is not initialized, proceed without tracking
            return this._onOpenInternal();
        }

        return await Sentry.startSpan(
            {
                name: 'ImapMailbox.syncWorkflow',
                attributes: {
                    // Account Context
                    account: this.connection?.account,
                    path: this.path,
                    ...
                    // Mailbox Context
                    syncTrigger: 'onOpen',
                    isFirstConnection: !this.previouslyConnected,
                    clientType: 'IMAP'
                },
    ...
    /**
     * Internal implementation of onOpen (extracted for Sentry tracking)
     * @private
     */
    async _onOpenInternal() {
        clearTimeout(this.runPartialSyncTimer);
        this.selected = true;
        ...
        try {
            ...
            // Add Sentry error tagging for sync workflow failures
            const Sentry = global.Sentry || null;
            if (Sentry) {
                Sentry.withScope(scope => {
                    // Error Tags
                    scope.setTag('operation', 'mailbox_sync_workflow');
                    scope.setTag('sync_phase', 'onOpen');
                    scope.setTag('thread_id', workerThreadId);
                    scope.setTag('process_id', process.pid);
                    scope.setTag('account', this.connection?.account);
                    scope.setTag('mailbox_path', this.path);
                    scope.setTag('error_category', err.mailboxMissing ? 'mailbox_missing' : 'sync_failure');
                    scope.setTag('worker_type', 'imap');
