...
class Mailbox {
    ...
    extractThreadRoot(headers) {
        ...
        if (referencesHeader && Array.isArray(referencesHeader) && referencesHeader.length > 0) {
            // References header format: ["<msg1> <msg2> <msg3>"]
            // The first element contains the entire references string, need to parse it
            let referencesString = referencesHeader[0];
        ...
        if (inReplyTo && Array.isArray(inReplyTo) && inReplyTo.length > 0) {
            let firstInReplyTo = inReplyTo[0];
            if (firstInReplyTo && typeof firstInReplyTo === 'string') {
                // Clean up the message ID and normalize
                let cleanRef = firstInReplyTo.trim();
                if (!cleanRef.startsWith('<')) cleanRef = '<' + cleanRef;
                if (!cleanRef.endsWith('>')) cleanRef = cleanRef + '>';
                return cleanRef.toLowerCase();
            }
        }
    ...
    /**
     * Checks if a message should be processed based on thread root tracking (with full headers)
     * Only processes messages that are part of threads initiated by this platform
     * @param {Object} messageInfo - Full message data with headers loaded
     * @returns {Promise<Boolean>} True if message should be processed
     */
    async shouldProcessMessageWithHeaders(messageInfo) {
        if (!messageInfo.headers) {
            // If no headers, this might be our own message, check messageId
            if (messageInfo.messageId) {
                const threadRootsKey = this.connection.getThreadRootsKey();
                const isOurMessage = await this.connection.redis.sismember(threadRootsKey, messageInfo.messageId.toLowerCase());
                return isOurMessage;
            }
            return false; // No headers and no messageId, skip
        }

        // Extract thread root from message headers
        let threadRoot = this.extractThreadRoot(messageInfo.headers);
    ...
    /**
     * Checks if a message should be processed based on thread root tracking
     * Only processes messages that are part of threads initiated by this platform
     * @param {Object} messageData - Basic message data with headers
     * @returns {Boolean} True if message should be processed
     */
    async shouldProcessMessage(messageData) {
        // If no headers available, we need to fetch them first
        if (!messageData.headers) {
            return true; // Let it proceed to fetch headers and then processNew will handle filtering
        }

        // Extract thread root from message headers
        let threadRoot = this.extractThreadRoot(messageData.headers);
        
        // If no thread root found, this is likely not a reply
        if (!threadRoot) {
            return false; // Skip - not part of any conversation
        }

        // Check if this thread root is in our tracked threads
        const threadRootsKey = this.connection.getThreadRootsKey();
        const isTrackedThread = await this.connection.redis.sismember(threadRootsKey, threadRoot);
    ...
    async processNew(messageData, options, canSync, storedStatus) {
        ...
        // console.log('requestedHeaders', requestedHeaders);
        let shouldProcessMessage = await this.shouldProcessMessageWithHeaders(messageInfo);
        // console.log('shouldProcessMessage', shouldProcessMessage);
                ...
                while (!fetchCompleted) {
                    try {
                        ...
                        for (let i = 0; i < messages.length; i++) {
                            ...
                            // Add breathing room for event loop - allow other operations to process
