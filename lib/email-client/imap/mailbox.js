...
class Mailbox {
    ...
    async processNew(messageData, options, canSync, storedStatus) {
        ...
        // check if message references any known thread messages
        let referenceIds = [ messageInfo.messageId ];
        for (let header of ['in-reply-to', 'references']) {
            for (let value of [].concat((messageInfo.headers || {})[header] || [])) {
                let matches = value.match(/<[^<>]+>/g);
                if (matches) {
                    referenceIds.push(...matches.map(m => m.toLowerCase()));
                }
            }
        }

        const threadKey = this.connection.getThreadMessagesKey();
        let inThread = false;
        if (referenceIds.length) {
            const hits = await this.connection.redis.smismember(threadKey, referenceIds);
            inThread = hits.some(Boolean);
        }
        ...
        if (messageInfo.messageId) {
            await this.connection.redis.sadd(threadKey, messageInfo.messageId);
        }
